...
```mermaid
graph TB
    MM["MemoryManager<br/><i>remember | recall | forget | get_context_window | run_gc</i>"]

    MM --> F["Fleeting Memory<br/><code>fleeting.py</code>"]
    MM --> ST["Short-Term Memory<br/><code>short_term.py</code>"]
    MM --> LT["Long-Term Memory<br/><code>long_term.py</code>"]

    F --- FD["dict store, key-value<br/>auto-clear on turn end"]
    ST --- STD["sliding window, max 100<br/>auto-summary, auto-prune<br/>refresh every 10 ops"]
    LT --- LTD["graph backend or fallback<br/>auto-decay 0.95^days<br/>consolidation, agent/run recall"]

    MM --> GC["MemoryGarbageCollector<br/><code>gc.py</code>"]
    GC --> GCO["auto_forget | summarize | prune_relevance<br/>prune_staleness | prune_dupes | improve_memory"]
    GC --> LH["Lifecycle Hooks"]
    LH --> H1["on_turn_end() -> clear fleeting"]
    LH --> H2["on_session_end() -> summarize + promote"]
    LH --> H3["on_idle() -> full maintenance cycle"]

    style MM fill:#0f3460,stroke:#e94560,color:#fff
